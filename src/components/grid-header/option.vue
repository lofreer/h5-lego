<template>
  <el-form ref="options-form" label-width="80px" size="mini">
    ...
    <template v-if="option.action">
      ...
      <template v-if="option.action.type === 'header-more-click'">
        <header-more-item :mores="option.action.config"></header-more-item>
      </template>
    </template>
  </el-form>
</template>
...
export default {
  ...
  components: {
    ...
    headerMoreItem: () => import("./headerMoreItem.vue"),
  },
  props: {
    option: {
      type: Object,
