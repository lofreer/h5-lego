<template>
  ...
  >
    <section v-show="component.style[0].val === 'scroll-x'" class="scroll-left">
      ...
      <div :id="`indicator-${component.domId}`" class="indicator">
        <div class="dotty"></div>
      </div>
    </section>
    ...
    <section
      ...
      class="no-scroll"
      :id="`scroll-two-${component.domId}`"
    >
      <div class="scroll-content">
        <div v-for="(item, idx) in scrolls" :key="idx" class="scroll-item">
          <a
            v-if="item.val"
            :href="
              item.click && item.click.href ? item.click.href : 'javascript:;'
  ...
  methods: {
    initScroll() {
      ...
      if (this.component.style[0].val === "scroll-x") {
        setTimeout(() => {
          this.leftScroll = new IScroll(`#scroll-one-${this.component.domId}`, {
            scrollX: true,
            scrollY: false,
            ...
            snapSpeed: 400,
            indicators: {
              el: document.querySelector(`#indicator-${this.component.domId}`),
              resize: false,
            },
          });
        }, 0);
        ...
        setTimeout(() => {
          this.noScroll = new IScroll(`#scroll-two-${this.component.domId}`, {
            scrollX: true,
            scrollY: false,
            eventPassthrough: true,
            preventDefault: false,
